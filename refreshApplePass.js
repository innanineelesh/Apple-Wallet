...
module.exports = async (req, res) => {
    ...
    // Generate the new token based on the current date and studentId
    const currentDate = new Date().toISOString();
    const newToken = `${currentDate}-${studentId}`;

    const passJsonPath = path.join(passDir, 'pass.json');
    let passJson;
    try {
        passJson = jsonfile.readFileSync(passJsonPath);
    } catch (err) {
        console.error('Error reading pass JSON:', err);
        return res.status(500).send('Error reading pass JSON');
    }

    // Update the token in the pass JSON
    const passData = JSON.parse(passJson.barcode.message);
    passData.token = newToken; // Update the token field
    passJson.barcode.message = JSON.stringify(passData);
    ...
    try {
        ...
        // Serve the updated pass to the user
        res.download(path.join(__dirname, passFileName), 'UpdatedStudentPass.pkpass');
    } catch (err) {
        console.error('Error refreshing pass package:', err);
        res.status(500).send('Error refreshing pass');
    }
};
